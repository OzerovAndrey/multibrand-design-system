...
type State = { user: User | null; balance: number; deposited: boolean; depositOpen: boolean };

const KEY = "mb-session";
const load = (): State => {
  try {
    const s = JSON.parse(localStorage.getItem(KEY) || "null");
    if (s?.user) return { user: s.user, balance: s.balance ?? 0, deposited: !!s.deposited, depositOpen: false };
  } catch { /* storage may be blocked */ }
  return { user: null, balance: 0, deposited: false, depositOpen: false };
};
...
const set = (patch: Partial<State>) => {
  state = { ...state, ...patch };
  try { localStorage.setItem(KEY, JSON.stringify({ user: state.user, balance: state.balance, deposited: state.deposited })); } catch { /* ignore */ }
...
export const signOut = () => set({ user: null, balance: 0, deposited: false, depositOpen: false });
